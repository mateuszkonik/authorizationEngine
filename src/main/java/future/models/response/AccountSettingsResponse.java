...
public class AccountSettingsResponse {
	...
	public AccountSettingsResponse(Users user, Boolean publishGaSecret) {
		email = user.getEmail();
		login = user.getLogin();
		...
		twoFactorAuthMethod = user.getTwoFactorMethod();
		enabledNotificationsTypes = user.getEnabledEotificationsTypes();
		notificationSound = user.getNotificationSound();
		phoneNumber = user.getPhoneNumber();

		if(publishGaSecret) {
			gaSecret = user.getGaSecret();
		}
	}
}
